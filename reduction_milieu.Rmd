...
date: "Création : 01/08/2022 - Modification : 30/01/2023"
output: 
  html_document:
    toc: TRUE
    toc_depth: 3
...
Cercle de corrélation (axe 1 et 2).

```{r,out.width="20%",fig.show='hold', echo=FALSE, warning=FALSE}
DV = tout[grep("cercle_correlation_axe1_2",tout)]
include_graphics(c(DV[grep("mai",DV)],DV[grep("juin",DV)],
                   DV[grep("juillet",DV)],DV[grep("aout",DV)],
                   DV[grep("septembre",DV)]))
``` 
...
Cercle de corrélation (axe 2 et 3).

```{r,out.width="20%",fig.show='hold', echo=FALSE, warning=FALSE}
DV = tout[grep("cercle_correlation_axes2_3",tout)]
include_graphics(c(DV[grep("mai",DV)],DV[grep("juin",DV)],
                   DV[grep("juillet",DV)],DV[grep("aout",DV)],
                   DV[grep("septembre",DV)]))
``` 
...
Qu'apporte cet axe par rapport à un MNT ?  
En comparant ce premier axe à des courbes de niveau, on remarque que l'axe est effectivement assez similaire aux courbes de niveau. Néanmoins, on observe des nuances au sein d'une même tranche d'altitude. De plus, le plateau de l'Arselle est plus différenciée que seulement par l'altitude. Ainsi, bien qu'extrement tiré par l'altitude, le premier axe de cet ACP retransmet aussi d'autres informations.  
Pour visualisation sur QGIS : C:/Users/perle.charlot/Documents/PhD/comparaison_mnt_axe1ACP.qgz
...
```{r, echo=FALSE,out.width="100%",fig.show='hold',message=FALSE,warning=FALSE}
paths_rast_ACP <- list.files(paste0(wd,"/output/ACP/",type_donnees,"/"),'.tif$',full.names = T,recursive=TRUE)
...
```{r,out.width="20%",fig.show='hold', echo=FALSE, warning=FALSE}
DI = tout[grep("graph_individus_",tout)]
include_graphics(c(DI[grep("mai",DI)],DI[grep("juin",DI)],
                   DI[grep("juillet",DI)],DI[grep("aout",DI)],
                   DI[grep("septembre",DI)]))
``` 
...
* Axe 1 :GDD et NDVI toujours dans 4 premières variables contribuant le plus --> étagement altitudinal inversé (haute altitude <-> valeur négative).

* Axe 2 : assez variable au cours des mois. Variable de CS bien présentes mais pas premières contributrices.

* Axe 3 : couloirs d'avalanche ? Mais attention car variable "présence avalanche" est loin d'être précise et exhaustive.
...
```{r,out.width="20%",fig.show='hold', echo=FALSE, warning=FALSE}
DI = tout[grep("graph_individus",tout)]
include_graphics(c(DI[grep("mai",DI)],DI[grep("juin",DI)],
                   DI[grep("juillet",DI)],DI[grep("aout",DI)],
                   DI[grep("septembre",DI)]))
``` 
