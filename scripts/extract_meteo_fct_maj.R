...
extract_meteo <- function(path_data_allslopes, years, variables_pro, 
                          variables_meteo,
  ...
  # variables_pro = liste des variables à utiliser issues chaine SAFRAN-CROCUS (dossier pro)
  # variable_meteo = liste des variables à utiliser issues chaine SAFRAN (dossier meteo)
  ...
  years = years_to_use
  variables_pro = var_to_use_pro
  variables_meteo = var_to_use_meteo
  ...
  pt_var_annee_p = array(data=NA, dim=c(length(unique(selected_points)),20,length(years))) # Mettre à jour le nombre de variables calculées
  pt_var_annee_m = array(data=NA, dim=c(length(unique(selected_points)),20,length(years))) # Mettre à jour le nombre de variables calculées

  # initialisation et récupération de la première année
  init_annee = 0
  # SAFRAN-CROCUS (pro)
  pro_file = paste0(path_data_allslopes, "/pro/PRO_",as.character(years[1]-1),"080106_",as.character(years[1]),"080106.nc")
  src <- tidync(pro_file)
  data_y1_p <- src %>% activate("D0,D3") %>%  hyper_tibble(select_var = variables_pro)
  # data_y1$TG4 <- (src %>% activate("D0,D2,D3") %>%  hyper_tibble(select_var = "TG4"))$TG4
  data_y1_p = data_y1_p[data_y1_p$Number_of_points %in% selected_points,]

  # SAFRAN (meteo)
  meteo_file = paste0(path_data_allslopes, "/meteo/FORCING_",as.character(years[1]-1),"080106_",as.character(years[1]),"080106.nc")
  src_m <- tidync(meteo_file)
  data_y1_m <- src_m %>% activate("D2,D0") %>%  hyper_tibble(select_var = variables_meteo)
  data_y1_m = data_y1_m[data_y1_m$Number_of_points %in% selected_points,]
  ...
  for (y in years){
    ...
    src_p2 <- tidync(pro_file)
    data_y2_p <- src_p2 %>% activate("D0,D3") %>%  hyper_tibble(select_var = variables_pro)
    # data_y2$TG4 <- (src %>% activate("D0,D2,D3") %>%  hyper_tibble(select_var = "TG4"))$TG4
    data_y2_p = data_y2_p[data_y2_p$Number_of_points %in% selected_points,]
    ...
    data_hiver_p = data_y1_p[which(data_y1_p$time >= deb_hiver & data_y1_p$time<=fin_hiver),]
    data_ete_p = rbind(data_y1_p[which(data_y1_p$time >= deb_ete),],data_y2_p[which(data_y2_p$time <= fin_ete),])
    ...
    for(no_point in 1:length(unique(selected_points))){
      ...
      pt_var_annee_p[no_point,1,init_annee] = mean(data_hiver$TS_ISBA[which(data_hiver$Number_of_points == unique(selected_points)[no_point])])

      #*---- colonne 2 : Température moyenne estivale de l'air sur la période ----
      # (la température correspond à la couche d'air au-dessus du sol ou de la neige)
      pt_var_annee_p[no_point,2,init_annee] = mean(data_ete$TS_ISBA[which(data_ete$Number_of_points == unique(selected_points)[no_point])])
      ...
      # On additionne les températures du sol à 8cm de profondeur à partir de la date de déneigement
      # On additionne les températures DE L'AIR à partir de la date de déneigement
      # et uniquement pour les jours où il n'y a pas de neige et où la température est supérieure à 5.5 °C
    ...
    data_y1_p = data_y2_p
    saveRDS(data_y1_p, file = paste0(output_path,"/data_y1_p.rds"))
    saveRDS(pt_var_annee_p, file = paste0(output_path,"/pt_var_annee_p.rds"))
