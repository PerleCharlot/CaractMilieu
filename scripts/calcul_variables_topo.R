...
if(!dir.exists(paste0(output_path,"/var_topo"))){
  dir.create(paste0(output_path,"/var_topo"))
}

# Calcul de la pente ----
pente25m <- terrain(MNT25m, opt="slope",unit="degrees",
                    filename=paste0(output_path,"/var_topo/pente_25m.tif"))
...
northing25m <- calc(exposition25m, fun = cos, 
                    filename= paste0(output_path,"/var_topo/northing_25m.tif")) 
easting25m <- calc(exposition25m, sin,
                   filename= paste0(output_path,"/var_topo/easting_25m.tif"))
# Sauvegarde de l'exposition en degrés
exposition25m_deg <- terrain(MNT25m, opt="aspect",unit="degrees",
                             filename=paste0(output_path,"/var_topo/exposition_25m.tif"))
...
writeRaster(TWI25m, filename=paste0(output_path,"/var_topo/TWI_25m.tif"))

# # Calcul du TPI ----
# TPI_100m <- terrain(MNT25m, opt="TPI",
#                     filename=paste0(output_pat,"/var_topo/TPI_25m.tif"))
# 
# 
# library(spatialEco)
# 
